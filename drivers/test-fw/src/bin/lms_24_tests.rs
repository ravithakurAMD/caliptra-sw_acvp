...
fn test_sigver_acvp() {
    ...
    // Safety: LmsPublicKey<6> and LmsSignature<6,51,15> are #[repr(C)] with
    // no padding (verified by static_assert in caliptra_lms_types).  All
    // fields use zerocopy integer types whose in-memory byte order matches the
    // LMS wire format (RFC 8554): U32<BigEndian> for algorithm-type fields and
    // U32<LittleEndian> via bytes_to_words_6 (which stores bytes unchanged)
    // for hash/nonce/path fields.  The buffer sizes equal the struct sizes.
